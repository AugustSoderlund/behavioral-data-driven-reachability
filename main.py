...
from utils.operations import visualize_zonotopes, create_M_w, input_zonotope, minkowski_sum, linear_map
...
import numpy as np
from utils.simulation import calc_d, generate_trajectory, simulate_forged_traj, _simulation, run_scenario, vis_class_trajs, format_volume_calc_for_latex, visualize_scenario, visualize_state_inclusion_acc, intersection_figure
...
def get_modal_zonotopes_for_scenarios():
    ...
    _dict = {1: ["cross_right", "cross_straight"], 
    ...
    for i in range(1,4):
        ...
        run_scenario(i)
    for i in range(1,4):
        try: visualize_scenario(scenario=str(i), all_modes=False, save=True, overlay_image=True)
        except Exception: print(f"Not possible to visualize any modal zonotopes for scenario {i}"); break
        plt.close("all")
        for mode in _dict[i]:
            try: 
                fig, ax = plt.subplots()
...
def get_state_inclusion_acc():
    """
        Code to reproduce the inclusion accuracy
        TODO: Update this with correct code
    """
    pass
